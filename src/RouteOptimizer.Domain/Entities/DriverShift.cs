...
public class DriverShift : AggregateRoot<Guid>
{
    ...
    private DriverShift(Guid id, Guid driverId, Guid vehicleId, Guid warehouseId, DateOnly shiftDate, DateTime? startedAt, DateTime? endedAt) : base(id)
        => (DriverId, VehicleId, WarehouseId, ShiftDate, StartedAt, EndedAt) = (driverId, vehicleId, warehouseId, shiftDate, startedAt, endedAt);
