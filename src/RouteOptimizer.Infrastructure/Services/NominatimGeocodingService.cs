...
public class NominatimGeocodingService : IGeocodingService
{
    ...
    public NominatimGeocodingService(IHttpClientFactory factory, IOptions<GeocodingSettings> settings)
    {
    ...
    public async Task<Result<GeoCoordinate>> GeocodeAsync(Address address, CancellationToken ct)
    {
        ...
        var url = $"search?q={query}&format=json&limit=1&countrycodes={Uri.EscapeDataString(_settings.CountryCodes)}";
